...
def visualizeBatchPair(options, config, inp_pair, detection_pair, indexOffset=0, prefix='', suffix='', write_ply=False, write_new_view=False):
    ...
    for pair_index, (input_dict, detection_dict) in enumerate(zip(inp_pair, detection_pair)):
        image_dict = visualizeBatchDetection(options, config, input_dict, detection_dict, indexOffset=indexOffset, prefix=prefix, suffix='_' + str(pair_index), prediction_suffix="_pred", write_ply=write_ply, write_new_view=write_new_view)
...
def visualizeBatchDetection(options, config, input_dict, detection_dict, indexOffset=0, prefix='', suffix='', prediction_suffix='', write_ply=False, write_new_view=False):
    ...
    if 'detection' in detection_dict and len(detection_dict['detection']) > 0:
        ...
        if 'flag' in detection_dict:
            ...
            detection_flags = {}
            pass
        instance_image, normal_image, depth_image = draw_instances(config, image, depth_gt, detections[:, :4], detection_masks > 0.5, detections[:, 4].astype(np.int32), detections[:, 6].astype(np.int32),  detections[:, 8:], detections[:, 5], draw_mask=True, transform_planes=False, detection_flags=detection_flags)
...
def draw_instances(config, image, depth, boxes, masks, semantic_ids, class_ids, parameters,
                      scores=None, title="",
    ...
    semantic_colors = ColorPalette(17).getColorMap(returnTuples=True)
    semantic_colors[0] = (128, 128, 128)
    ...
    for i in range(N):
        ...
        ## Mask
        mask = masks[:, :, i]
        masked_image = apply_mask(masked_image.astype(np.float32), mask, semantic_colors[semantic_id]).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    legend = np.zeros((680, 100, 3))

    for i in range(17):
        legend[i*40 : (i+1)*40, :, :] = semantic_colors[i]
