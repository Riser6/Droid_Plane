...
def evaluate(options):
    ...
    for name, detector in detectors:
        ...
        for sampleIndex, sample in enumerate(data_iterator):
            print(options.numTestingImages)
            ...
            if 'inference' not in options.dataset:
                ...
                for c in range(len(detection_pair)):
                    np.save(options.test_dir + '/' + str(sampleIndex % 500) + '_plane_parameters_' + str(c) + '.npy', detection_pair[c]['detection'][:, 6:9].cpu().numpy())
                    np.save(options.test_dir + '/' + str(sampleIndex % 500) + '_plane_masks_' + str(c) + '.npy', detection_pair[c]['masks'][:, 80:560].cpu().numpy())
                    continue
                pass
