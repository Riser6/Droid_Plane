...
class FPN(nn.Module):
    def __init__(self, C1, C1_d, C2, C3, C4, C5, out_channels, bilinear_upsampling=False):
        ...
        self.sagates = nn.ModuleList([
            SAGate(in_planes=256, out_planes=256, bn_momentum=0.1),
            SAGate(in_planes=512, out_planes=512, bn_momentum=0.1),
            SAGate(in_planes=1024, out_planes=1024, bn_momentum=0.1),
            SAGate(in_planes=2048, out_planes=2048, bn_momentum=0.1)
        ])
...
class SAGate(nn.Module):
    def __init__(self, in_planes, out_planes, reduction=16, bn_momentum=0.0003):
        ...
        self.fsp_d = FSP(in_planes, out_planes, reduction)

        self.gate_rgb = nn.Conv2d(in_planes*2, 1, kernel_size=1, bias=True)
        self.gate_d = nn.Conv2d(in_planes*2, 1, kernel_size=1, bias=True)
...
class DualBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1_d = nn.Conv2d(inplanes, planes, kernel_size=1, stride=stride)
        self.bn1_d = nn.BatchNorm2d(planes, eps=0.001, momentum=0.01)
        self.padding2_d = SamePad2d(kernel_size=3, stride=1)
        self.conv2_d = nn.Conv2d(planes, planes, kernel_size=3)
        self.bn2_d = nn.BatchNorm2d(planes, eps=0.001, momentum=0.01)
        self.conv3_d = nn.Conv2d(planes, planes * 4, kernel_size=1)
        self.bn3_d = nn.BatchNorm2d(planes * 4, eps=0.001, momentum=0.01)
        self.relu_d = nn.ReLU(inplace=True)
        self.downsample_d = downsample
    ...
    def forward(self, x):
        ...
        out2 = self.conv1_d(x2)
        out2 = self.bn1_d(out2)
        out2 = self.relu_d(out2)

        out2 = self.padding2_d(out2)
        out2 = self.conv2_d(out2)
        out2 = self.bn2_d(out2)
        out2 = self.relu_d(out2)

        out2 = self.conv3_d(out2)
        out2 = self.bn3_d(out2)

        if self.downsample is not None:
            residual2 = self.downsample_d(x2)
...
class ResNet_Dual(nn.Module):
    ...
    def stages(self):
        return [self.C1, self.C1_d, self.C2, self.C3, self.C4, self.C5]
...
def detection_target_layer(proposals, gt_semantic_ids, gt_class_ids, gt_boxes, gt_masks, gt_parameters, config):
    ...
    gt_semantic_ids: [batch, MAX_GT_INSTANCES] Integer class IDs.
    ...
    rois: [batch, TRAIN_ROIS_PER_IMAGE, (y1, x1, y2, x2)] in normalized
          coordinates
    target_semantic_ids: [batch, TRAIN_ROIS_PER_IMAGE]. Integer class IDs.
    ...
    gt_semantic_ids = gt_semantic_ids.squeeze(0)
    ...
    if positive_roi_bool.sum() > 0:
        ...
        roi_gt_semantic_ids = gt_semantic_ids[roi_gt_box_assignment.data]
    ...
    if positive_count > 0 and negative_count > 0:
        ...
        if config.GPU_COUNT:
            zeros = zeros.cuda()
        roi_gt_semantic_ids = torch.cat([roi_gt_semantic_ids, zeros], dim=0)
        ...
        roi_gt_semantic_ids = Variable(torch.IntTensor(), requires_grad=False)
        ...
        if config.GPU_COUNT:
            rois = rois.cuda()
            roi_gt_semantic_ids = roi_gt_semantic_ids.cuda()
    ...
    return rois, roi_gt_semantic_ids, roi_gt_class_ids, deltas, masks, roi_gt_parameters
...
def refine_detections(rois, semantic_probs, probs, deltas, parameters, window, config, return_indices=False, use_nms=1, one_hot=True):
    ...
    if len(semantic_probs.shape) == 1:
        semantic_ids = semantic_probs.long()
    else:
        _, semantic_ids = torch.max(semantic_probs, dim=1)
        pass
    ...
    if len(probs.shape) == 1:
        ...
        semantic_scores = semantic_probs[idx, semantic_ids.data]
        class_scores = probs[idx, class_ids.data]
        deltas_specific = deltas[idx, semantic_ids.data]
    ...
    keep_bool = semantic_ids > 0

    ## Filter out low confidence boxes
    if config.DETECTION_MIN_CONFIDENCE and False:
        keep_bool = keep_bool & (semantic_scores >= config.DETECTION_MIN_CONFIDENCE)
    ...
    if use_nms == 2:
        ## Apply per-class NMS
        pre_nms_semantic_ids = semantic_ids[keep.data]
        pre_nms_semantic_scores = semantic_scores[keep.data]
        pre_nms_class_ids = class_ids[keep.data]
        pre_nms_scores = class_scores[keep.data]
        pre_nms_rois = refined_rois[keep.data]

        ixs = torch.arange(len(pre_nms_semantic_ids)).long().cuda()
        ## Sort
        ix_rois = pre_nms_rois
        ix_scores = pre_nms_semantic_scores
    ...
    top_ids = semantic_scores[keep.data].sort(descending=True)[1][:roi_count]
    keep = keep[top_ids.data]
    #print('num detectinos', len(keep))

    ### Apply plane anchors
    class_parameters = config.applyAnchorsTensor(class_ids, class_parameters)
    ## Arrange output as [N, (y1, x1, y2, x2, semantic_id, semantic_score, class_id, class_score, parameters)]
    ## Coordinates are in image domain.
    result = torch.cat((refined_rois[keep.data],
                        semantic_ids[keep.data].unsqueeze(1).float(),
                        semantic_scores[keep.data].unsqueeze(1),
...
def detection_layer(config, rois, mrcnn_semantic, mrcnn_class, mrcnn_bbox, mrcnn_parameter, image_meta, return_indices=False, use_nms=1, one_hot=True):
    ...
    return refine_detections(rois, mrcnn_semantic, mrcnn_class, mrcnn_bbox, mrcnn_parameter, window, config, return_indices=return_indices, use_nms=use_nms, one_hot=one_hot)
...
class Classifier(nn.Module):
    def __init__(self, depth, pool_size, image_shape, num_semantics, num_classes, num_parameters, debug=False):
        ...
        self.linear_semantic = nn.Linear(1024, num_semantics)

        self.linear_bbox = nn.Linear(1024, num_semantics * 4)
    ...
    def forward(self, x, rois, ranges, pool_features=True, gt=None):
        ...
        mrcnn_semantic_logits = self.linear_semantic(x)
        mrcnn_semantic_probs = self.softmax(mrcnn_semantic_logits)
        ...
        if pool_features:
            return [mrcnn_semantic_logits, mrcnn_semantic_probs, mrcnn_class_logits, mrcnn_probs, mrcnn_bbox, mrcnn_parameters, roi_features]
        else:
            return [mrcnn_semantic_logits, mrcnn_semantic_probs, mrcnn_class_logits, mrcnn_probs, mrcnn_bbox, mrcnn_parameters]
...
class Mask(nn.Module):
    def __init__(self, config, depth, pool_size, image_shape, num_classes):
        ...
        self.pool_size = pool_size
        self.image_shape = image_shape
        self.num_classes = config.NUM_SEMANTICS
        ...
        self.conv5 = nn.Conv2d(256, config.NUM_SEMANTICS + config.NUM_PARAMETER_CHANNELS, kernel_size=1, stride=1)
    ...
    def forward(self, x, rois, pool_features=True):
        ...
        x = self.relu(x)
        x = self.conv5(x)
        # self.config.NUM_PARAMETER_CHANNELS : 0
...
def compute_losses(config, rpn_match, rpn_bbox, rpn_class_logits, rpn_pred_bbox, target_semantic_ids, mrcnn_semantic_logits, target_class_ids, mrcnn_class_logits, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask, target_parameters, mrcnn_parameters):

    #print(rpn_match.shape)  #torch.Size([1, 102300, 1])
    #print(rpn_class_logits.shape)   #torch.Size([1, 102300, 2])
    rpn_class_loss = compute_rpn_class_loss(rpn_match, rpn_class_logits)

    #print(rpn_bbox.shape)           #torch.Size([1, 256, 4])
    #print(rpn_match.shape)          #torch.Size([1, 102300, 1])
    #print(rpn_pred_bbox.shape)      #torch.Size([1, 102300, 4])

    rpn_bbox_loss = compute_rpn_bbox_loss(rpn_bbox, rpn_match, rpn_pred_bbox)

    """print("11111111111111111111")
    print(target_class_ids.shape)   #torch.Size([200])
    print(mrcnn_class_logits.shape) #torch.Size([200, 8])
    print(target_semantic_ids.shape)   #torch.Size([200])
    print(mrcnn_semantic_logits.shape) #torch.Size([200, 19])"""

    mrcnn_semantic_loss = compute_mrcnn_class_loss(target_semantic_ids, mrcnn_semantic_logits)


    mrcnn_class_loss = compute_mrcnn_class_loss(target_class_ids, mrcnn_class_logits)

    """print("222222222222222222")
    print(target_deltas.shape)      #torch.Size([200, 4])
    print(target_class_ids.shape)   #torch.Size([200])
    print(mrcnn_bbox.shape)         #torch.Size([200, 19, 4])"""

    mrcnn_bbox_loss = compute_mrcnn_bbox_loss(target_deltas, target_semantic_ids, mrcnn_bbox)

    """print("3333333333333333")
    print(target_mask.shape)    #torch.Size([200, 28, 28])
    print(target_parameters.shape)  #torch.Size([200, 3])
    print(mrcnn_mask.shape) #torch.Size([200, 19, 28, 28])
    print(mrcnn_parameters.shape)   #torch.Size([200, 8, 3])"""

    mrcnn_mask_loss = compute_mrcnn_mask_loss(config, target_mask, target_semantic_ids, target_parameters, mrcnn_mask)
    mrcnn_parameter_loss = compute_mrcnn_parameter_loss(target_parameters, target_class_ids, mrcnn_parameters)
    return [rpn_class_loss, rpn_bbox_loss, mrcnn_semantic_loss, mrcnn_class_loss, mrcnn_bbox_loss, mrcnn_mask_loss, mrcnn_parameter_loss]
...
class MaskRCNN(nn.Module):
    ...
    def build(self, config):
        ...
        resnet = ResNet_Dual("resnet101", stage5=True, numInputChannels=config.NUM_INPUT_CHANNELS)
        C1, C1_d, C2, C3, C4, C5 = resnet.stages()

        ## Top-down Layers
        ## TODO: add assert to varify feature map sizes match what's in config
        self.fpn = FPN(C1, C1_d, C2, C3, C4, C5, out_channels=256, bilinear_upsampling=self.config.BILINEAR_UPSAMPLING)
        ...
        ## FPN Classifier
        self.debug = False
        self.classifier = Classifier(256, config.POOL_SIZE, config.IMAGE_SHAPE, config.NUM_SEMANTICS, config.NUM_CLASSES, config.NUM_PARAMETERS, debug=self.debug)
    ...
    def load_weights(self, filepath):
        ...
        if os.path.exists(filepath):
            ...
            for k,v in state.items():
                if "_d." in k:
                    raw_k = k.replace("_d", "")
                    if raw_k in state_dict:
                        if k == "fpn.C1_d.0.weight":
                            state_dict[k] = torch.mean(state_dict[raw_k], 1, True)
                        else:
                            state_dict[k] = state_dict[raw_k]
    ...
    def predict(self, input, mode, use_nms=1, use_refinement=False, return_feature_map=False):
        molded_images, droid_depth = input[0]
        image_metas = input[1]
        ...
        ## Feature extraction
        [p2_out, p3_out, p4_out, p5_out, p6_out] = self.fpn(input[0])  #torch.Size([1, 256, 160, 160]) torch.Size([1, 256, 10, 10])
        ...
        rpn_class_logits, rpn_class, rpn_bbox = outputs     #torch.Size([1, 102300, 2])  torch.Size([1, 102300, 2])  torch.Size([1, 102300, 4])
        ...
        if mode == 'inference':
            ...
            #print("********************************")
            #print(gt_class_ids.shape)   #torch.Size([1, 13])
            #print(gt_boxes.shape)   #torch.Size([1, 13, 4])
            #print(gt_masks.shape)   #torch.Size([1, 13, 56, 56])
            #print(gt_parameters.shape)  #torch.Size([1, 13, 3])
            #rpn_rois: torch.Size([1, 2000, 4]) 

            rois, target_semantic_ids, target_class_ids, target_deltas, target_mask, target_parameters = \
                detection_target_layer(rpn_rois, gt_semantic_ids, gt_class_ids, gt_boxes, gt_masks, gt_parameters, self.config)      #torch.Size([200, 4]) torch.Size([200]) torch.Size([200]) torch.Size([200, 4]) torch.Size([200, 28, 28]) torch.Size([200, 3])

            if len(rois) == 0:
                mrcnn_semantic_logits = Variable(torch.FloatTensor())
                mrcnn_semantic = Variable(torch.IntTensor())
                mrcnn_class_logits = Variable(torch.FloatTensor())
                mrcnn_class = Variable(torch.IntTensor())
                mrcnn_bbox = Variable(torch.FloatTensor())
                mrcnn_mask = Variable(torch.FloatTensor())
                mrcnn_parameters = Variable(torch.FloatTensor())
                if self.config.GPU_COUNT:
                    mrcnn_semantic_logits = mrcnn_semantic_logits.cuda()
                    mrcnn_semantic = mrcnn_semantic.cuda()
                    mrcnn_class_logits = mrcnn_class_logits.cuda()
                    mrcnn_class = mrcnn_class.cuda()
                    mrcnn_bbox = mrcnn_bbox.cuda()
                    mrcnn_mask = mrcnn_mask.cuda()
                ...
                ## Network Heads
                ## Proposal classifier and BBox regressor heads
                mrcnn_semantic_logits, mrcnn_semantic, mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_parameters, roi_features = self.classifier(mrcnn_feature_maps, rois, ranges, pool_features=True)
                """
                print("*********************************")
                print(mrcnn_semantic.shape)        torch.Size([200, 19])
                print(mrcnn_class.shape)        torch.Size([200, 8])
                print(mrcnn_bbox.shape)         torch.Size([200, 19, 4])
                print(mrcnn_parameters.shape)   torch.Size([200, 8, 3])
                print(roi_features.shape)       torch.Size([200, 320, 7, 7])
                """
                ## Create masks for detections
                mrcnn_mask, _ = self.mask(mrcnn_feature_maps, rois) #torch.Size([200, 19, 28, 28])
                pass
            ...
            if use_refinement:  # False
                ...
                mrcnn_semantic_logits_final, mrcnn_semantic_final, mrcnn_class_logits_final, mrcnn_class_final, mrcnn_bbox_final, mrcnn_parameters_final = mrcnn_semantic_logits, mrcnn_semantic, mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_parameters
                
                rpn_rois = rois
                detections, indices, _ = detection_layer(self.config, rpn_rois, mrcnn_semantic_final, mrcnn_class_final, mrcnn_bbox_final, mrcnn_parameters_final, image_metas, return_indices=True, use_nms=use_nms) #torch.Size([23, 11]) torch.Size([23])
                ...
                if len(detections) > 0:
                    ...
                    """print("**************")
                    print(detection_boxes.shape)    #torch.Size([1, 32, 4])
                    print(detection_masks.shape)    #torch.Size([32, 19, 28, 28])
                    print(roi_features.shape)       #torch.Size([32, 320, 7, 7])"""
            ...
            if len(detections) > 0:
                ...
                if len(detections) > 0:
                    ...
                    positive_overlaps = overlaps[positive_indices.data,:]
                    roi_gt_box_assignment = torch.max(positive_overlaps, dim=1)[1]
                    roi_gt_boxes = gt_boxes[roi_gt_box_assignment.data,:]   #torch.Size([11, 4])

                    roi_gt_class_ids = gt_class_ids[roi_gt_box_assignment.data]     #torch.Size([11])
                    roi_gt_parameters = gt_parameters[roi_gt_box_assignment.data]   #torch.Size([11, 3])
                    roi_gt_parameters = self.config.applyAnchorsTensor(roi_gt_class_ids.long(), roi_gt_parameters)  #torch.Size([11, 3])

                    ## Assign positive ROIs to GT masks
                    roi_gt_masks = gt_masks[roi_gt_box_assignment.data,:,:]     #torch.Size([11, 56, 56]
            ...
            info = [rpn_class_logits, rpn_bbox, target_semantic_ids, mrcnn_semantic_logits, target_class_ids, mrcnn_class_logits, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask, target_parameters, mrcnn_parameters, detections, detection_masks, roi_gt_parameters, roi_gt_masks, rpn_rois, roi_features, roi_indices]
            """
            print(rpn_class_logits.shape)   torch.Size([1, 102300, 2])
            print(rpn_bbox.shape)           torch.Size([1, 102300, 4])
            print(target_class_ids.shape)   torch.Size([200])
            print(mrcnn_class_logits.shape) torch.Size([200, 8])
            print(target_deltas.shape)      torch.Size([200, 4])
            print(mrcnn_bbox.shape)         torch.Size([200, 8, 4])
            print(target_mask.shape)        torch.Size([200, 28, 28])
            print(mrcnn_mask.shape)         torch.Size([200, 8, 28, 28])
            print(target_parameters.shape)  torch.Size([200, 3])
            print(mrcnn_parameters.shape)   torch.Size([200, 8, 3])
            print(detections.shape)         torch.Size([15, 9])
            print(detection_masks.shape)    torch.Size([15, 8, 28, 28])
            print(roi_gt_parameters.shape)  torch.Size([15, 3])
            print(roi_gt_masks.shape)       torch.Size([15, 224, 224])
            print(rpn_rois.shape)           torch.Size([200, 4])
            print(roi_features.shape)       torch.Size([15, 320, 7, 7])
            print(roi_indices.shape)        torch.Size([15])
            """
...
if __name__ == '__main__':
    ...
    ## Top-down Layers
    ## TODO: add assert to varify feature map sizes match what's in config
    fpn = FPN(C1, C1_d, C2, C3, C4, C5, out_channels=256, bilinear_upsampling=self.config.BILINEAR_UPSAMPLING)